...
| `prompt_injection` | Private có 20/80 câu `GHI CHU KHACH` chèn giá giả 1.000.000 VND; test xác nhận cả 20 input sạch không còn note/giá giả (`prv-006`, `prv-011`, `prv-080`). | Nội dung khách hàng nằm cùng kênh với chỉ dẫn điều khiển agent. | Prompt coi note là dữ liệu không tin cậy; wrapper loại `ORDER:` và toàn bộ note injection trước `call_next`. |
